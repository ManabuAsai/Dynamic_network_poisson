function [vTh_new,iA] = DRAM_dnar1w(vTheta,mTheta,j,mS1,mY,mDY,mWDY,iPar,vMu0,mLam0,vLam_ini)
    %iNs = 0.5*iN*(iN+1);
    ...
    iX0 = loglike_dnar1w(mY,mDY,mWDY,vTheta,iPar,vLam_ini)+LogPrior(vTheta,vMu0,mLam0);
    iG0 = iX0-iY0;
    iI=100;
    ...
    for i=1:iI
        ...
        ll = loglike_dnar1w(mY,mDY,mWDY,vTh_c,iPar,vLam_ini);
        %h=0;
        %while (h<1)
        %    vTh_c = vD + sqrt(gam)*mS1h*random('Normal',0,1,iP,1);
        %    %vTh_c(iP)
